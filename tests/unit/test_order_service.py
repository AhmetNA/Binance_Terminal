...
class TestOrderService(unittest.TestCase):
    ...
    def test_place_buy_order(self, mock_data_manager, mock_calc_qty, mock_symbol_info, mock_get_price, mock_balance):
    ...
    @patch('services.orders.order_service.data_manager')
    def test_place_sell_order(self, mock_data_manager, mock_calc_qty, mock_symbol_info, mock_get_price, mock_asset):
    ...
    def test_execute_order(self, mock_order_manager, mock_order_type, mock_preferences, mock_client):
        ...
        # Assertions
        self.assertEqual(result['orderId'], 12345)
        mock_manager_instance.execute_order.assert_called_once_with("Hard_Buy", self.test_symbol, "MARKET", None)
    
    @patch('services.orders.order_service.prepare_client')
    @patch('services.orders.order_service.execute_order')
    @patch('services.orders.order_service.retrieve_usdt_balance')
    @patch('services.orders.order_service.data_manager')
    def test_make_order(self, mock_data_manager, mock_balance, mock_execute, mock_client):
