...
class TestEndToEndTrading(unittest.TestCase):
    ...
    def test_complete_buy_sell_cycle(self):
        ...
        usdt_before = float([b for b in account_before['balances'] if b['asset'] == 'USDT'][0]['free'])
        btc_before = float([b for b in account_before['balances'] if b['asset'] == 'BTC'][0]['free'])
        
        self.assertEqual(usdt_before, 1000.0)
        self.assertEqual(btc_before, 0.0)
        
        # Phase 2: Execute buy order
        buy_order = self.mock_client.order_market_buy(symbol='BTCUSDT', quantity='0.001')
        ...
        self.mock_client.get_account.return_value['balances'][0]['free'] = '950.0'  # USDT reduced
        self.mock_client.get_account.return_value['balances'][1]['free'] = '0.001'  # BTC increased
        
        # Phase 3: Check state after buy
        account_after_buy = self.mock_client.get_account()
        usdt_after_buy = float([b for b in account_after_buy['balances'] if b['asset'] == 'USDT'][0]['free'])
        btc_after_buy = float([b for b in account_after_buy['balances'] if b['asset'] == 'BTC'][0]['free'])
        
        self.assertEqual(usdt_after_buy, 950.0)
        self.assertEqual(btc_after_buy, 0.001)
        
        # Phase 4: Execute sell order
        sell_order = self.mock_client.order_market_sell(symbol='BTCUSDT', quantity='0.001')
        ...
        self.mock_client.get_account.return_value['balances'][0]['free'] = '1001.0'  # USDT increased
        self.mock_client.get_account.return_value['balances'][1]['free'] = '0.0'  # BTC reduced
        
        # Phase 5: Check final state
        account_after_sell = self.mock_client.get_account()
        usdt_after_sell = float([b for b in account_after_sell['balances'] if b['asset'] == 'USDT'][0]['free'])
        btc_after_sell = float([b for b in account_after_sell['balances'] if b['asset'] == 'BTC'][0]['free'])
    ...
    def test_multiple_trades_scenario(self):
        ...
        for i in range(3):
            order = self.mock_client.order_market_sell(symbol='BTCUSDT', quantity='0.001')
            trades_executed.append({
                'order_id': order['orderId'],
                'side': order['side'],
                'quantity': float(order['fills'][0]['qty']),
    ...
    def test_error_recovery_scenario(self):
        ...
        self.mock_client.order_market_buy.side_effect = BinanceAPIException(
            response={'code': -1013, 'msg': 'Filter failure: LOT_SIZE'},
            status_code=400
        )
    ...
    def test_portfolio_rebalancing_scenario(self):
        ...
        # Check again
        btc_deviation = abs(initial_portfolio['BTC']['current_percentage'] - initial_portfolio['BTC']['target_percentage'])
    ...
    def test_risk_management_scenario(self):
        ...
        order = self.mock_client.order_market_buy(symbol='BTCUSDT', quantity='0.0006')  # ~$30
        self.assertEqual(order['status'], 'FILLED')
