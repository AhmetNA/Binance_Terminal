...
def change_order_type(new_order_type: str) -> bool:
    ...
    try:
        ...
        if success:
            logger.info(
                f"✅ Successfully changed order type from {current_type} to {new_order_type}"
            )
...
def toggle_order_type() -> str:
    ...
    try:
        ...
        if success:
            ...
            logger.error("❌ Failed to toggle order type")
            return current_type
...
def get_effective_order_type() -> str:
    ...
    if _SESSION_ORDER_TYPE_OVERRIDE is not None:
        logging.debug(
            f"🎯 Using session override order type: {_SESSION_ORDER_TYPE_OVERRIDE}"
        )
...
def clear_session_order_type():
    ...
    if _SESSION_ORDER_TYPE_OVERRIDE is not None:
        logging.info(
            f"🧹 Clearing session order type override: {_SESSION_ORDER_TYPE_OVERRIDE}"
        )
        _SESSION_ORDER_TYPE_OVERRIDE = None
    else:
