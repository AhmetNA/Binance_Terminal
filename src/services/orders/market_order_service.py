...
def place_market_buy_order(symbol: str, amount_or_percentage: float, amount_type: str = 'percentage', client=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"🔄 Placing MARKET BUY order: {quantity} {order_params.symbol} at ${current_price}")
        ...
        trade_data = {
            ...
            'type': f'${usdt_to_spend:.2f}_Market_Buy' if amount_type.lower() == 'usdt' else f'{amount_or_percentage*100:.0f}%_Market_Buy',
            'quantity': quantity,
            'price': current_price,
        ...
        avg_price = float(order.get('cummulativeQuoteQty', 0)) / float(executed_qty) if float(executed_qty) > 0 else 0
        
        logger.info(f"✅ {order_type} BUY order completed: {symbol}")
        logger.info(f"   📊 Status: {order_status} | Miktar: {executed_qty} | Ortalama Fiyat: {avg_price:.8f}")
...
def place_market_sell_order(symbol: str, amount_or_percentage: float, amount_type: str = 'percentage', client=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if amount_type.lower() == 'usdt':
            ...
            logger.info(f"💰 Converting ${amount_or_percentage:.2f} to {quantity_to_sell} {symbol} (≈{actual_percentage*100:.2f}% of balance)")
        else:
            # Percentage kullan
        ...
        logger.info(f"🔄 Placing MARKET SELL order: {quantity} {order_params.symbol} at ${current_price}")
        ...
        trade_data = {
            ...
            'symbol': order_params.symbol,
            'side': order_params.side.value,
            'type': f'${amount_or_percentage:.2f}_Market_Sell' if amount_type.lower() == 'usdt' else f'{amount_or_percentage*100:.0f}%_Market_Sell',
            'quantity': quantity,
            'price': current_price,
        ...
        avg_price = float(total_received) / float(executed_qty) if float(executed_qty) > 0 else 0
        
        logger.info(f"✅ {order_type} SELL order completed: {symbol}")
        logger.info(f"   📊 Status: {order_status} | Satılan: {executed_qty} | Ortalama Fiyat: {avg_price:.8f}")
        logger.info(f"   💰 Amount Type: {amount_type} | Input: {amount_or_percentage}")
        logger.info(f"   💰 Toplam Alınan: {total_received} USDT | Order ID: {order.get('orderId')}")
