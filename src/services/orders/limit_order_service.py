...
def place_limit_buy_order(symbol: str, amount_or_percentage: float, limit_price: float, amount_type: str = 'percentage', 
    ...
    try:
        ...
        trade_data = {
            ...
            'type': f'${usdt_to_spend:.2f}_Limit_Buy' if amount_type.lower() == 'usdt' else f'{amount_or_percentage*100:.0f}%_Limit_Buy',
            'quantity': quantity,
            'price': final_price,
        ...
        error_msg = f"❌ LIMIT BUY order error for {symbol}: {e}"
        logger.error(error_msg)
        logger.exception("Full traceback for limit buy order error:")
        raise
...
def place_limit_sell_order(symbol: str, amount_or_percentage: float, limit_price: float, amount_type: str = 'percentage',
    ...
    try:
        ...
        error_msg = f"❌ LIMIT SELL order error for {symbol}: {e}"
        logger.error(error_msg)
        logger.exception("Full traceback for limit sell order error:")
        raise
...
def cancel_order(symbol: str, order_id: int, client=None) -> Dict[str, Any]:
    ...
    try:
        ...
        error_msg = f"❌ Cancel order error for {symbol}, order {order_id}: {e}"
        logger.error(error_msg)
        logger.exception("Full traceback for cancel order error:")
        raise
...
def get_open_orders(symbol: Optional[str] = None, client=None) -> list:
    ...
    try:
        ...
        error_msg = f"❌ Get open orders error: {e}"
        logger.error(error_msg)
        logger.exception("Full traceback for get open orders error:")
        raise
