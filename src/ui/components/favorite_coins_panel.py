...
class FavoriteCoinPanel(BaseComponent):
    ...
    def _create_trading_buttons(self):
        ...
        try:
            ...
            for col in range(FAVORITE_COIN_COUNT):
                btn = self._create_order_button("Hard Buy", HARD_BUY_STYLE, "Hard_Buy", col)
                self.layout.addWidget(btn, 0, col)
            
            # Row 1: Soft Buy buttons
            for col in range(FAVORITE_COIN_COUNT):
                btn = self._create_order_button("Soft Buy", SOFT_BUY_STYLE, "Soft_Buy", col)
            ...
            for col in range(FAVORITE_COIN_COUNT):
                btn = self._create_order_button("Soft Sell", SOFT_SELL_STYLE, "Soft_Sell", col)
                self.layout.addWidget(btn, 3, col)
            
            # Row 4: Hard Sell buttons
            for col in range(FAVORITE_COIN_COUNT):
                btn = self._create_order_button("Hard Sell", HARD_SELL_STYLE, "Hard_Sell", col)
    ...
    def _create_order_button(self, text, style, operation_type, coin_index):
        ...
        btn.clicked.connect(lambda: self._handle_order_button(operation_type, coin_index))
        return btn
