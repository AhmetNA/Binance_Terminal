"""
core/trading_operations.py
Core trading operations that break circular dependencies.
This module contains shared functionality used by both services and models.
"""
...
def log_order_amount(amount_or_percentage: float, amount_type: str, balance: Optional[float] = None) -> None:
    """
...
def validate_trading_parameters(symbol: str, side: str, amount_or_percentage: float, amount_type: str) -> None:
    """
    ...
    if amount_or_percentage <= 0:
        raise ValueError(f"amount_or_percentage must be positive, got: {amount_or_percentage}")
    
    # Percentage specific validation
    if amount_type.lower() == 'percentage' and amount_or_percentage > 1.0:
        raise ValueError(f"Percentage amount cannot be greater than 1.0, got: {amount_or_percentage}")
...
class OrderExecutionContext:
    ...
    def __init__(self, symbol: str, side: str, amount_or_percentage: float, 
                 amount_type: str, order_type: str = "MARKET", limit_price: Optional[float] = None):
        """
        ...
        logging.info(f"🔧 Created OrderExecutionContext: {self.side} {self.symbol} "
